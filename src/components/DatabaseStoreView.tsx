...
export const DatabaseStoreView: React.FC = () => {
  ...
  const handleDeleteTenant = async (sessionId: string) => {
    const savedPass = sessionStorage.getItem('db_store_passcode') || 'kunal@123as$';
                    ...
                    {filteredLogs.length === 0 ? (
                      ...
                      filteredLogs.map((l, i) => (
                        <tr key={l.id || i} className="hover:bg-cyan-500/5 transition">
                          <td className="py-3 px-4 text-gray-400 text-[11px]">{new Date(l.timestamp).toLocaleTimeString()}</td>
                          <td className="py-3 px-4">
                            <span
